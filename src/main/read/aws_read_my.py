...
class S3Reader:
    ...
    def list_files(self, s3_client, bucket_name,folder_path):
        try:
            list_file=[]
            for obj in s3_client.Bucket(bucket_name).objects.filter(Prefix=folder_path):
                if not obj.key.endswith('/'):
                    list_file.append(f"s3://{(bucket_name)}/{(obj.key)}")
            logger.info("Total files available in folder '%s' of bucket '%s': %s", folder_path, bucket_name, list_file)
            return list_file
